...
The predicate \coqe{lin} is defined by mutual recursion with \coqe{single_x}:
\begin{itemize}
    ...
    \item \coqe{single_x (P : Process bool TMT) : Prop} recursively checks that if newly created channels are unmarked, a single marked channel is found. Once that channel is found, it uses \coqe{lin} to check that continuing down recursively finds no marked channels.
\end{itemize}
