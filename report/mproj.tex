...
\todo{
   Since your mechanisation is based on the continuation-passing style
   encoding, I'm wondering if you should add a section Encoding at the end
   of background, which you can take from my journal Session Types
   Revisited at Info and Comput 2017 (you need to cite this too, btw). You
   also need to cite Kobayashi extended version of Type Systems for
   Concurrent Programs, where there was a proposition of such encoding.
   Again, you can find this reference in my journal.

   What you can do is have separate sections of session types, linear
   types and encoding, even though the last two might be short sections.
   Look at how I did it in Session Types Revisited, and follow that, so
   that you don't need to "invent" a new way of presenting it. You can of
   course get inspired from the text in the journal, as well, as long as
   you don't copy paste (avoid plagiarism! :)
}
...
The \coqe{CComp} rules define commutativity and associativity for process compositition, and structural congruence to \emph{go under} composition.

\coqi[57-64]{../src/Processes.v}

The \coqe{CScope} rules express scope expansion, scope commutativity, and session type commutativity, and allow structural congruence to \emph{go under} scope restriction.

\coqi[66-78]{../src/Processes.v}

Finally, \coqe{COutCong} and \coqe{CInCong} allow structural congruence to \emph{go under} input and output.

\coqi[80-86]{../src/Processes.v}

\todo{we defined reflexivity and transitivity separately to make proof search easier}
\todo{we didn't manage to add symmetry}
\todo{this involves a <-> proof}
\todo{the linearity predicate does not make it easy for scope expansion}
\todo{the only affected thing is scope expansion which is now directional}
\todo{so we have encoded a subset of structural congruence, it is still sound}
...
\todo{mention structural congruence}
