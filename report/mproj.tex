...
Parametric polymorphism allows programs to depend on unconstrained types. Terms inhabiting these types cannot possibly be constructed nor eliminated by pattern matching, and are therefore said to be \emph{opaque}. In a purely functional language like Coq, programs of an unconstrained polymorphic type follow important theorems that stem just from the polymorphism of the type \cite{Wadler1989}.

In our project, we parametrise several inductive definitions with polymorphic types. We do so by using the \coqe{Section Processes.} directive, instructing Coq to parametrise with parameters \coqe{ST} and \coqe{MT} all the definitions within the section. These variables are only used by the inductive type \coqe{Message}, which is then used by the other several definitions within the section. The type \coqe{Message} captures the values sent between processes, and is indexed by the type of the value that is sent. The two constructors \coqe{V} and \coqe{C} construct values of a given base type and channels of a given session type, respectively.

\coqi[6-13]{../src/Processes.v}

To prevent channels from being constructed outside of the operations provided by the calculus, the channel constructor expects a token of the unknown type \coqe{ST}. That is, channels cannot be constructed as long as the type \coqe{ST} is not made concrete.

The linearity predicate introduced in \S \ref{linearity} is defined recursively on processes. Processes contain functions that take messages as arguments and give back processes; to be able to traverse processes where messages passing is modelled as function abstraction, one has to be able to provide messages of any given type. For messages containing channels, \coqe{ST} can be set to the unit type, or to any other inhabited type. For messages containing base types however, values of these base types need to be provided. To circumvent this, we wrap base types with a function on types \coqe{MT}: to produce a message with a value of the base type \coqe{M}, we require a value of the type \coqe{MT M}. If \coqe{MT} is set to \coqe{id}, then a value of the original type must be provided. If, however, \coqe{MT} is set to \coqe{fun t => unit}, then no matter what the original type is, a value of the unit type will be required. 
