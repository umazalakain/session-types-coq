...
\paragraph{Scope} This section covers a subset of session types: the diadic (shared by two processes), finate (no replication or recursion), deterministic (no indeterministic choice), and synchronous session types.
...
Session-typed channels impose typing rules on the syntactical constructs of the \picalc{}: a process can perform an action on a channel only if that channel is capable of that action. \todo{introduce rules} \todo{linearity} \todo{explain $\circ$}
