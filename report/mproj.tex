...
\todo{mention wen's work, francisco ferreira locally nameless, evan about linear types}
\todo{compare our work to theirs}
...
The present work can be expanded in several different directions:

\todo{expand on these}
\begin{itemize}
    \item Extending structural precongruence to structural congruence.

    \item Including shared types from the standard \picalc{}.

    \item Supporting replicating session types and processes.

    \item Supporting recursive session types and processes.

    \item Formalising our operational semantics.
\end{itemize}

As part of a wider effort, I intend to continue working on the machine verification of process calculi during my oncoming PhD. Some of the aspects I plan to work on are encoding session types into more primitive types in simpler calculi, formalising and machine verifying the properties of the correspondences between session types and linear logic, formalising the more advanced extensions to session types, and translating these representations into other proof assistants. 
